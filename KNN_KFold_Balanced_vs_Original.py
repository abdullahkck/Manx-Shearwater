...
def main():
    ...
    #  -------------------------------------------------------------------------
    dataset_name_ori = "Calls_67_pairs_X_encoded_30e_(4x40x30)"
    pickle_in = open(dataset_name_ori + ".pickle", "rb")
    X_ori = pickle.load(pickle_in)
    X_ori = X_ori.reshape(len(X_ori), 4800)
    ...
    for train, test in k_fold.split(inputs, targets):
        ...
        for k in range(1, 10):
            ...
            y_pred = clf.predict(inputs_ori)

            # rounded_targets = np.argmax(targets[test], axis=1)

            cnf_matrix = confusion_matrix(targets_ori.argmax(axis=1), y_pred.argmax(axis=1), labels=[0, 1, 2])
            ...
            test_score = clf.score(inputs_ori, targets_ori)
